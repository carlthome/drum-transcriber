...
T = size(pX, 2);
numberOfStates = mc.nStates();
numberOfObservations = size(pX, 2);
q = mc.InitialProb;
A = mc.TransitionProb;
B = pX;
...
% Initialization
alfaHat = [];
initAlfaTemp = zeros(numberOfStates, numberOfObservations);
for j = 1:numberOfStates
    initAlfaTemp(j) = q(j) * B(j, 1);
end
c(1) = sum(initAlfaTemp(:, 1));
for j = 1:numberOfStates
...
% Handle finite duration
if (mc.finiteDuration())
    c(end+1) = sum(alfaHat(:, T) .* A(:, numberOfStates+1));
end
